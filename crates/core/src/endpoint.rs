...
//!
//! Lives in core (rather than `linkpilot-ipc`) so that core types — most
//! notably [`crate::daemon::DaemonRuntime`] when it builds a doctor
//! report — can resolve the daemon's socket path without pulling in the
//! ipc crate (which depends on core, so the reverse direction would
//! cycle). `linkpilot_ipc::path` re-exports everything here, so existing
...
impl Endpoint {
    /// Human-readable rendering for log lines / doctor reports.
    pub fn display(&self) -> String {
        match self {
            Endpoint::UnixSocket(p) => p.display().to_string(),
            Endpoint::NamedPipe(s) => s.clone(),
        }
    }
}
