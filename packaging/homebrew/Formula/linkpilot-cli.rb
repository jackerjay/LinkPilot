...
#
# Ships two binaries — `lp` and `linkpilot-daemon`. `lp` alone would
# work for local-mode commands, but `lp daemon install` writes a
...
class LinkpilotCli < Formula
  ...
  def caveats
    <<~EOS
      ...
      To register `lp` as the system default browser handler you need the
      ...
      To run the daemon at login (so `lp open` works system-wide):
        lp daemon install
    EOS
  end
